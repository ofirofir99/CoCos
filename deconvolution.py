...
def decompose_to_channels_batched_only_gpu(crops_with_high_intensity, psfs, rounded_peaks, number_of_rl_iters=30,
                                           rl_batch_size=200000, scatter_add_batch_size=100000,
                                           should_compute_final_image=True,number_of_channels=3):
    """
    ...
    all_ch_crops = cp.zeros(
        (crops_with_high_intensity.shape[0], crops_with_high_intensity.shape[1], crops_with_high_intensity.shape[2], 3))
    ...
    for ch in range(number_of_channels):
        ...
        deconvolved_images = richardson_lucy_batched_gpu(crops_with_high_intensity, psfs[:, :, ch],
                                                         num_iter=number_of_rl_iters, clip=False,
                                                         batch_size=rl_batch_size)

        start_col, end_col = (deconvolved_images.shape[2] - 3) // 2, (
                deconvolved_images.shape[2] + 3) // 2  # array operations
        ...
        if should_compute_final_image:
            for i in range(0, len(chCrops), scatter_add_batch_size):
                ...
                cupyx.scatter_add(decomposed_image[:, :, ch], (rows[batch_idx], cols[batch_idx]),
                                  chCrops[batch_idx])  # adding many pics at once, on gpu - way faster than looping.
            print(time.time() - t)
...
def richardson_lucy_batched_gpu(images, psf, num_iter=50, clip=False, filter_epsilon=None, batch_size=100):
    """
    ...
    for start_idx in range(0, number_of_images_images, batch_size):
        ...
        for _ in range(num_iter):
            ...
            im_deconv *= fftconvolve(relative_blur, psf_mirror, mode='same', axes=(1, 2))
