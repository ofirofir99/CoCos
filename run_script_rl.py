...
def main():
    ...
    ref_red_name = "Ref640_N250_LPN2_TL8_TR1389p5_TCon100_TCoff15_TD5p56_TA0_Gain500_64010.tif"
    ref_green_name = "Ref560_N250_LPN2_TL8_TR1389p5_TCon100_TCoff15_TD5p56_TA0_Gain500_56010.tif"
    ref_blue_name = "Ref488_N250_LPN2_TL8_TR1389p5_TCon100_TCoff15_TD5p56_TA0_Gain500_488I10.tif"
    ...
    with tifffile.TiffFile(rgb_path) as tif:
        ...
        stack = tif.asarray()
    all_crops, all_crops_max, stack_mean, stack_std = get_crops_all_pictures(stack, peaks)

    flattened_P = np.array(flatten(peaks))
    high_intensity_crops_index = np.where(all_crops_max > stack_mean + 6 * stack_std)[
        0]  # this is not always good, may return way too many crops. #TODO: improve
    ...
    t = time.time()
    decomposed_image_batched_new, all_convolved_crops_new = decompose_to_channels_batched_only_gpu(
        crops_with_high_intensity, psfs, rounded_peaks)
    print("get decomposed time:" + str(time.time() - t))

    # get ism image!
    t = time.time()
    final_ism_opt = run_ism(all_convolved_crops=all_convolved_crops_new, M=8, peaks=peaks_with_high_intensity,
